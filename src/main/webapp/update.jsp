<%@page import="com.csw.entity.Emp"%>
		...
		<div id="top_content">
			...
			<div id="content">
				...
				<%
					...
					request.setAttribute("p", session2.getAttribute("ems"));
				%>
				<form action="${path }/emp/emp_UpdateAction" method="post"
					...
					<table cellpadding="0" cellspacing="0" border="0"
						...
						<tr>
							...
							<td valign="middle" align="left"><input type="text"
								class="inputgri" name="id" value="${p.id }" readonly /></td>
						</tr>
						<tr>
							<td valign="middle" align="right">name:</td>
							<td valign="middle" align="left"><input type="text"
								class="inputgri" name="name" value="${p.name }" id="username"
								onblur="unFout()" /> <span id="sp1" style="color:red;"></span></td>
						</tr>
						<tr>
							<td valign="middle" align="right">salary:</td>
							<td valign="middle" align="left"><input type="text"
								class="inputgri" name="salary" value="${p.salary }" id="salary"
								onblur="saFout()" /> <span id="sp2" style="color:red;"></span></td>
						</tr>
						<tr>
							<td valign="middle" align="right">age:</td>
							<td valign="middle" align="left"><input type="text"
								class="inputgri" name="age" value="${p.age }" id="age"
								onblur="agFout()" /> <span id="sp3" style="color:red;"></span></td>
						</tr>
						<tr>
							<td valign="middle" align="right">deptName:</td>
							<td><select name="deptss">
									<c:forEach items="${listdept }" var="m">
										<c:if test="${m.deptid==p.dept.deptid}">
											<option value="${m.deptid}" selected="selected">${p.dept.deptname}
												${p.dept.deptid}</option>
										</c:if>
										<c:if test="${m.deptid!=p.dept.deptid}">
											<option value="${m.deptid}">${m.deptname}{m.deptid}</option>
										</c:if>
									</c:forEach>
							</select></td>
						</tr>
